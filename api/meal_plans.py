...
async def get_meal_plan(
    ...
    Uses authed_client so RLS correctly filters to only this user's rows.
    """
    logger.info(f"Fetching meal plan for user {current_user.id} from {start_date} to {end_date}")
    try:
        plan_response = authed_client.table("meal_plans").select(
            "*, planned_meals(*, meals(*, nutrient_profiles(*)))"
        ).eq("user_id", str(current_user.id)).gte("start_date", start_date).lte("end_date", end_date).execute()
