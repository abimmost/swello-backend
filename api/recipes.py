...
@router.post("")
async def create_recipe(request: CreateRecipeRequest, current_user: Any = Depends(get_current_user)):
    """
    ...
    try:
        # 1. Insert the parent meal record
        meal_response = supabase.table("meals").insert({
            "title": request.title,
        ...
        # 2. Insert the recipe linked to the meal
...
async def bookmark_recipe(recipe_id: str, current_user: Any = Depends(get_current_user)):
    """Saves a recipe to the user's bookmarks."""
    logger.info(f"Bookmarking recipe {recipe_id} for user {current_user.id}")
    try:
        response = supabase.table("bookmarks").upsert({
